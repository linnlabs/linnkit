...
export class PreprocessorPipeline {
  ...
  async process(messages: AiMessage[]): Promise<PreprocessorPipelineResult> {
    ...
    for (const preprocessor of preprocessors) {
      ...
      try {
        ...
        // 收集应用的策略
        result.appliedStrategies.forEach(strategy => appliedStrategies.add(strategy));
        ...
        console.warn(`Preprocessor ${preprocessor.name} failed:`, error);
      }
    }
  ...
  private debug(message: string, data?: Record<string, unknown>): void {
    if (this.context.debugMode) {
      console.log(`[PreprocessorPipeline] ${message}`, data);
    }
  }
}
 ...
 * 
 * 🔥 这里定义了预处理管道的默认配置：
 * 1. HistoryPurificationPreprocessor (priority: 1) - 历史净化，最先执行
 * 
 * 💡 扩展示例:
 * 2. PIIScrubbingPreprocessor (priority: 2) - PII数据清洗
 * 3. FormatNormalizationPreprocessor (priority: 3) - 格式标准化
 */
export function createDefaultPreprocessorRegistry(): PreprocessorRegistry {
  const registry = new PreprocessorRegistry();
  
  // 注册历史净化预处理器 - 优先级1，最先执行
  // 🔥 使用共享实现，配置 Chat 模式的日志前缀
  registry.register(new HistoryPurificationPreprocessor({ logPrefix: 'Chat-HistoryPurification' }));
  
  // 注册工具历史过滤器 - 优先级2，移除Agent工具消息
  registry.register(new ToolHistoryFilterPreprocessor());

  // 注册用户引用寿命控制器 - 优先级3，清理过期引用
  registry.register(
  ...
  // 未来可以在这里注册更多预处理器:
  // registry.register(new PIIScrubbingPreprocessor());
  // registry.register(new FormatNormalizationPreprocessor());
  // registry.register(new DeduplicationPreprocessor());
 ...
 * @returns 配置好的预处理管道实例
 */
export function createDefaultPreprocessorPipeline(
  context: PreprocessorContext = {}
): PreprocessorPipeline {
  const registry = createDefaultPreprocessorRegistry();
