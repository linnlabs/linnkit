...
本文重点是 **Agent profile 的详细上下文构建机制**。三阶段模型、示意图、协议不变量和工具组一致性规则以当前实现为准。
...
├─────────────────────────────────────────────────┤
│  [checkpoint 工具组]                             │ ← summary + 可选 host payload
│    assistant: context_checkpoint(...)           │   agent 的 2-5 句过渡摘要
│    tool: { _type, summary, ...hostData }        │   扩展数据由 host 自己解释
├─────────────────────────────────────────────────┤
│  [checkpoint 之后的所有消息]                      │ ← 不做裁剪，仅受 token 预算约束
│    host_state_tool(...)                         │   可选的 host 状态工具
...
> Agent 调用 `context_checkpoint(summary, ...hostArgs)` 时写入的过渡摘要（2-5 句话）就在工具调用参数里，
> AI 下一轮可以直接看到这个工具组。工具调用记录本身就是摘要锚点，无需额外的 system 摘要消息。
>
> host 可以通过 hook 增加自己的 payload，但 framework 不解释这些字段，也不会把外部状态系统隐式热注入上下文。
> Checkpoint 后 AI 看到的完整信息链为：
> **checkpoint 工具组（过渡摘要 + 可选 host payload）+ 最近工具交互（连续性）→ 完整接班**。
...
Context Manager 只裁剪 messages。Graph 集成以 prepared model 的正式 route 作为总窗口和最大输出基线；Agent policy 只有显式声明 `maxTokens / reservedForResponse` 时才能进一步收窄。无显式 cap 时直接使用 route；无模型 route 的独立 Context Manager 接入才使用 256K/16K framework fallback。有效输入预算还要扣除 prepared Tool definitions，`ContextBuildResult.tokenUsage` 明确保存 `messageBudget / inputBudget / toolDefinitionTokens`，不再由 orchestration metadata 复制一份。

system reminder 和 provider tools 位于 Context Manager 边界之外，因此最终 Prompt 占用由 Graph 的 `measure_prompt_usage` stage 负责。它在 provider attempt 成功前按 System prompt、Conversation、Tool definitions 三项测量；只有成功 attempt 的严格 `ContextUsageSnapshot` 才能进入 checkpoint。Context Manager trace、telemetry 和 component ledger 都不能替代这份执行事实。
...
- 候选消息携带附件时，摘要调用只使用消息文本；原消息被摘要替换后，附件随消息退出活动上下文
- 摘要与 checkpoint 都不删除 durable event 或资源，只改变本次及后续 LLM 输入可见的消息集合
...
- `RuntimeEvent(tool_output).observation` 是模型唯一文本视图，`data` 是程序化事实；转为 AiMessage 后分别位于 `content` 与 `metadata.data`。checkpoint 只读取正式 `metadata.data`，不保存原始 JSON 影子副本。
