...
你要复用的是 `linnkit`（即 `packages/linnkit/src/*`），然后自己实现一个新的 host layer；不要复制 `src/app-hosts/linnya/*` 再硬改。
...
### 3.1 `packages/linnkit/src/testkit/*`
...
- `LinnyaEventStoreAdapter` 是 host-owned adapter，不进入 `packages/linnkit/src/*` 公开面；它的职责是把 linnkit `append/range/latestEventId` 语义桥接到宿主 SQLite 实现。
...
- `packages/linnkit/src/index.ts:5` 看当前根入口兼容导出的 `withLLMTelemetryContext`
...
- `packages/linnkit/src/docs/engine/13-public-api-surface-and-host-migration-batches.md`
- `packages/linnkit/src/docs/engine/14-stable-vs-compat-exports.md`
- `packages/linnkit/src/docs/engine/19-d2-implementation-runbook.md`
- `packages/linnkit/src/docs/engine/20-d3-d4-port-interfaces-plan.md`
- `packages/linnkit/src/docs/engine/21-host-port-adapter-research.md`
