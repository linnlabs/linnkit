...
- 框架演进路线图、ADR 决策档案、隐患台账等**仓库内部档案**在 `docs/framework/` 与 `docs/archive/`——0.5.0 起这些目录**不在 npm tarball 内**；外部接入方读 [`docs/integration/`](./integration/) 即可
...
- `system-reminder/*`
  - runtime 级系统提醒拼装与约束
- `run-supervisor/*`
  - RunSupervisor 本体（`DefaultRunSupervisor` / `RunHandle` / `MemoryRunRegistryStore`）+ RunRegistryStore port + contract 测试。外部接入参考 [`docs/integration/run-supervisor.md`](./integration/run-supervisor.md)；内部协议演进追溯 `docs/framework/04-protocol-roadmap.md` N-3（仅仓库内）
- `telemetry/*`
...
- `profiles/chat/*`
  ...
  - chat 兼容层冻结计划详见仓库内部 `docs/framework/07-roi-ranked-priorities.md` Phase F（chat 兼容收敛 + 删除迁移期 compat namespace）
...
└── src/
    ...
    ├── integration/             # 接入手册集（按主题拆分的 17 个 .md）
...
下一阶段框架自身要研究的（仅仓库内部档案，不在 npm tarball）：

- 协议层下沉（`MessageBus` / `Memory` / `Permission` 等；`AgentSpec` 已于 0.5.0 落地）
- chat 兼容层何时彻底冻结、统一收敛到"tools-disabled agent"形态（详见 `docs/framework/07-roi-ranked-priorities.md` Phase F）
...
1. [`packages/linnkit/docs/integration/`](./integration/)（建议从 `integration/README.md` 进入）
2. [`packages/linnkit/src/runtime-kernel/README.md`](../src/runtime-kernel/README.md)
...
- 外部接入指南：[`packages/linnkit/docs/integration/`](./integration/)
...
- [`packages/linnkit/docs/integration/`](./integration/) —— 接入手册集，按主题拆分

### linnkit 框架文档

- `docs/framework/` —— **仓库内部活文档**（不在 npm tarball）：linnkit 作为独立 Agent 框架的演进
- `docs/99-research-notes/` —— **仓库内部**：外部项目调研笔记
- `docs/archive/engine-phases/` —— **已归档**：早期抽包决策档案
- [`packages/linnkit/docs/release/`](./release/) —— 发布流水（RELEASE.md + 历史叙事；在 tarball 内）
