...
> 上一阶段的 living doc 是 `docs/proposals/agent-architecture-upgrade-and-evolution-plan.md` 与 `docs/proposals/agent-package-boundary-extraction-proposal.md`，主要解决 **Phase 0~C：物理收口与 host/product 边界外置**。  
...
判断原则（继承自 `../00-vision-and-split.md` §3.2 + `[00-engine-scope-audit.md](./00-engine-scope-audit.md)` §1.1 强化版）：
...
- linnsec 正式产品开发前置全部就位 → 进入 [`secretary/01`](../secretary/01-product-vision-and-phased-direction.md) §9 三个待决策问题的拍板
...
1. **升级 engine** —— 写进 engine 升级清单，按 `docs/proposals/agent-architecture-upgrade-and-evolution-plan.md` 流程执行
...
- **secretary 侧**：进入 [`secretary/README.md`](../secretary/README.md) 推动 linnsec 立项（需先拍板 [`secretary/01 §9`](../secretary/01-product-vision-and-phased-direction.md) 三个待决策问题）。
