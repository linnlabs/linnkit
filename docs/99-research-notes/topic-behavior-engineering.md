...
- **内层** tick-pipeline（`graph-engine/executor.ts` 104–126）：`prepare_call → build_context → apply_system_reminder → execute_llm → build_decision`。
...
| benchmark（仅 dev/test） | required_tools_coverage / evidence_citation / tool_success_rate / 关键词 / 规划-分包质量 | 离线打分 + LLM Judge | 重 AI | `src/app-hosts/linnya/adapters/flow/run-hooks/benchmarkRunHook.ts`、`src/testkit/default-agent-benchmark/scoring/rules/shared.ts` |
...
3. benchmark 的 hard-rules **本身就是一份"零 AI 可计算信号目录"**，但只跑离线。**离线能算的信号，没有一条被搬进实时循环。**
4. `contextTrace` 默认关闭（`DEFAULT_CONTEXT_POLICY.contextTrace.enabled: false`）。

**结论**：linnkit 已经有"离线信号目录"（benchmark）+ "in-loop 反馈通道"（system-reminder），但两者没打通；且 in-loop 只覆盖第一类信号里最浅的一层。
...
第三条尤其有价值——它是"零 AI 测内容可信度"的少数真实可行点，且 benchmark 已有 `evidence_citation` 规则（`src/testkit/default-agent-benchmark/scoring/rules/shared.ts`），只差搬进实时循环。
