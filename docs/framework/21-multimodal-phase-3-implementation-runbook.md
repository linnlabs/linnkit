...
因此生产装配应在 routes/app-level workflow 创建一个 workspace-scoped materializer，再显式注入 root 和 child 的 `LlmCaller`。不得把 DB、workspace root 或 resolver 放进全局 Model Catalog、全局 `AIEngineImpl` 或 adapter descriptor。
