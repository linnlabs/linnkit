...
| `budgetPercentage` | `0.12` | 摘要文本本身的 token 长度上限占比 | ✅ 已开放 |
...
#### 1.2.9 `reasoningRetention`（🆕 1F 新增分组）

兑现"保留最近几轮 reasoning_content"的诉求：

| 字段 | 默认 | 含义 |
|------|------|------|
| `keepLatestThoughts` | `1` | 工作记忆里保留的最近 thought 消息数量（原 `MAX_THOUGHTS_TO_KEEP`）|

> **明确分层**：`reasoningRetention` 只处理"哪些 thought 进入最终上下文"这个 token 选择问题；Provider continuation replay 由 `inference_route.continuation.tool_replay` 声明，避免把模型协议与 Agent token 策略混在一起。
...
| F1.8 `reasoningRetention.keepLatestThoughts` 替换 `MAX_THOUGHTS_TO_KEEP` 硬编码 | 0.5d | F1.0-F1.5 | ✅ 已完成：`contextPolicyToContextBuilderConfig()` 映射 `keepLatestThoughts -> MAX_THOUGHTS_TO_KEEP`，`AgentWorkingMemoryProvider` 已按配置保留指定数量 thought |
...
| R7 | **移除 `MAX_THOUGHTS_TO_KEEP` 硬编码**：现有 host 默认值是 1，改成 0 / 多 会让 LLM 输入剧变 | 现有 host 行为漂移 | framework default 保持 1；migration note 写清 |
...
3. **行为层（testkit 不变量，新增 11 条）**
   ...
   - `keepLatestThoughts` 实际生效（thought 数量受限）
