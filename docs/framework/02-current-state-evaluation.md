...
### 2.5 Context Engineering 已经进入协议化阶段

`AgentSpec.contextPolicy` 已经从最初 3 个粗粒度分组扩展成真正可控的上下文工程协议面：
...
关键价值不是"字段多"，而是每个会影响最终 LLM 输入 token 的机制，都应该能被 host 声明、被 framework 合并、被 trace 解释、被 testkit 证明。

### 2.6 两类 checkpoint 严格区分

- **执行控制层 `Checkpointer`**：用于程序恢复，存 graph state、pending tool calls、待 resume 节点。
- **上下文工程层 checkpoint marker**：用于理解恢复，存 agent 主动提交的阶段摘要和替换来源。

0.6.0 候选线进一步补了 host-neutral 的 `ContextCheckpointTool` / `createContextCheckpointTool()`：linnkit 提供最小工具和协议 marker，host 仍决定是否注册、是否扩展 TaskState / SharedMemory / 文件写入。
