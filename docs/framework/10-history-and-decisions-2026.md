...
| K | `agent/utils/toolInteractionGroup.ts:277` | 函数名 `findCurrentRoundStartIndex` 用 "Round"，实际语义是 "Run"（以 `user_input` 切）；与即将引入的 `RunSupervisor` 命名 inconsistent | ✅ | N-1.3（`findCurrentRunStartIndex` 已落地，旧名 deprecated alias 保留 1 sprint） |
