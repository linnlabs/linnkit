...
4. **不破坏现有"机制层"不变量**——三阶段填充模型、ToolPairMatcher、ToolHistoryCompressor、replacementSourceIds、SummarizationProvider 协议**完全保留**
...
- 只做 Phase A（剥离）：linnsy 进入 S5 后会在 host 层自己造一套围栏机制——这正是 [`INTEGRATION_GUIDE §7`](../INTEGRATION_GUIDE.md) 第 1 条 "不要先造 bridge 再开发" 反对的
...
| 不变量 | 来源 | 本次改动是否破坏 |
|---|---|---|
| 三阶段填充模型（Must-Keep / Working Memory / Summarization） | `context-manager/README §核心架构` | ✗ 不破坏，仅把"isCoreMessage 写死的 if"替换为 MustKeepPolicy 表驱动 |
...
| ContextCheckpoint 工具组保护 | `context-manager/README §阶段二 P1-Checkpoint` | ✗ 一行代码不动 |
...
> **关键**：本次整治**只动 host 产品语义那一层**，linnkit 真正做对的"机制层"原封不动。
...
- **状态**：⚠️ **设计阶段**——抽象 / 取舍 / 风险定型；具体 task 拆解、迁移顺序、回归测试矩阵在 [`09-context-engineering-package-boundary-plan.md`](./09-context-engineering-package-boundary-plan.md)
- **下一步**：评审通过本文 → 启动 Phase A 实施（详见 09 文档）
- **触发上线时机**：必须在 linnsy 启动 S5 之前（详见 §1.2 时机判断）
