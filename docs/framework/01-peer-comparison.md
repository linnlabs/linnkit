...
| **上下文工程** | 三阶段 + working memory + `replacementSourceIds` + checkpoint marker + AgentSpec contextPolicy | 三层注入 + `defer_loading` | `ContextManager.reference_context_item` + diff 注入 | `on_pre_compress` 钩子 + 8 backend | 文件 + vector |
