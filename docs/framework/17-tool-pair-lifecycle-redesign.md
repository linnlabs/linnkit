...
- turn/run 与工具组定义：`src/context-manager/profiles/agent/utils/toolInteractionGroup.ts`
...
### 7.4 thoughts 保留对 turn 内缓存的影响（待观察项）⚠️

有两类"推理"载体，缓存影响完全不同：

- **`reasoning_details`（provider sidecar，挂在 `tool_calls` / `final_answer` 上）**：随宿主消息保留，2-turn 窗口内不被重复处理 → **缓存友好**，本次改动不影响它。
- **独立 `thought` 消息（`type='thought'`，对应流式 `reasoning_content`）**：由 `TextConversationRetention` 按 `MAX_THOUGHTS_TO_KEEP=1`（`config.ts`）**全局只保留最新 1 条**，更旧的每轮 drop。

关键链路（`runtime-kernel/llm/streaming-adapter.ts`）：推理模型在每个工具决策步骤前会流式产出 reasoning，`ThoughtStreamSegmenter` 在遇到正文/`tool_calls` 边界时把它 finalize 成一条**独立 `thought` 事件**，该事件持久化、进上下文（`shouldEnterAgentContext` 放行非空 thought），native 模式下被 `MessageFormatter` 输出成 `<think>...</think>` 的 assistant 消息发给 provider。

由此推导 turn 内缓存代价：

- 若模型走"每步一条独立 thought"路径，一个 N 步 turn 内会累积 `t_1..t_N`（各自夹在对应 `tool_calls` 之前）。
- `MAX_THOUGHTS_TO_KEEP=1` 是**全局且移动的**上限：第 N 次构建留 `t_{N-1}`，第 N+1 次构建留 `t_N`、把 `t_{N-1}` drop 掉。
- 被 drop 的 `t_{N-1}` 位于历史中部（在 `tool_calls_{N-1}` 之前），删除它使前缀在该位置错位 → **本该只重算尾部一组，实际会从 `t_{N-1}` 位置起多重算约一组**（若该组含大 input，代价更明显）。

也就是说，`MAX_THOUGHTS_TO_KEEP=1` 对"会流式产出独立 thought 的推理模型"是 turn 内缓存的一个**隐性抖动源**：它既不是"全保留"也不是"全不留"，而是"保留最新 1 条"这个**每步都在移动的目标**，恰好是缓存最不友好的形态。

两个都能消除抖动的稳定形态（作为未来杠杆，不绑进 T1–T6）：

- **(A) 当前 turn 内 thought 全保留**：turn 内前缀完全稳定，代价是多占 token（thought 通常较长）。
- **(B) 工具循环里不保留独立 thought，只留终局（final_answer 前）那条**：靠 `reasoning_details` 满足回放协议；前缀稳定且省 token。

**待观察 / 后续项**：先测一下当前生产模型是否真的每步产出独立 thought（若推理全走 `reasoning_details`，则本问题不存在）。若存在抖动，倾向把 thought 保留做成 **turn-scoped**（与 D1 的 turn 口径天然一致）：当前 turn 内 thought 稳定处理、只在 turn 边界对更旧 turn 的 thought 收敛。这是收益点也是一个明确的观察指标。
...
| 观察项 · thoughts 抖动（7.4） | 👀 待观察：确认生产模型是否每步产出独立 thought，再决定是否 turn-scoped |
