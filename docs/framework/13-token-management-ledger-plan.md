...
**顺带清理（同批做掉）**：删除孤儿 `src/shared/utils/llmTelemetryContext.ts`——它是旧 OpenAI 三字段版，已被 linnkit canonical 版 `packages/linnkit/src/shared/llmTelemetryContext.ts`（含 `CanonicalLlmUsage` / cache·reasoning 拆分）取代，全仓**无任何代码 import**（仅自身与一处 benchmark 计划 md 文本提及）。统一到 linnkit 版，消除重复实现与"改一份漏另一份"的风险。
...
| C26 | host | ✅ 删孤儿 `src/shared/utils/llmTelemetryContext.ts`，更新 `DEFAULT_AGENT_ULTRA_LONG_BENCHMARK_PLAN.md` 引用，统一到 linnkit canonical 版 | — | 全仓无残留 import；typecheck / 测试绿 |
