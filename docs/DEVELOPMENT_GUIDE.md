...
> **agent-only 约定**：`context-manager/profiles/chat/*` 已删除。纯聊天、补全、翻译、摘要这类单轮能力都应在 host 层注册为 tools-disabled agent，并通过 fence / options 把产品字段转换成通用 agent 输入。不要重新引入 chat profile 或 chat task resolver。
...
6. token 预算、摘要触发、工具历史截断等上下文决策必须走 `TokenizerPort` / `state.tokens` 这条统一口径；不要在 context-manager 功能代码里直接调用 `TokenCalculator`，否则 host 自定义 tokenizer 与 calibration 会失效。
...
当前 context-manager 没有已知的 skip 测试。长对话、摘要和回放属于事故高发区，禁止用 `skip` 绕过失败；无法稳定执行的场景应先提取确定性 port/harness。
...
详见 [`packages/linnkit/docs/README.md §4.5`](./README.md) 的对比表。
...
4. [`packages/linnkit/docs/README.md`](./README.md) —— linnkit 文档总入口
5. [`packages/linnkit/docs/framework/`](./framework/) —— 框架演进活文档
6. [`packages/linnkit/docs/integration/`](./integration/) —— 接入手册集（按主题拆分）
7. [`packages/linnkit/docs/integration/token-management.md`](./integration/token-management.md) —— token 口径、估算、账本与校准
