...
> - 2026-04-24 v4：**0.1.1 patch release** —— D-5 schemas detach round 2 落地 + 0.1.0 manifest URL 瑕疵兑现修正。**核心结果**：linnkit 现在真正自包含，src 与 dist 都不再依赖 `@app/schemas` 任何一个符号，dist 自然更小（runtime-kernel.cjs 407 KB → 384 KB，-23 KB），未来"linnkit 独立成 `linnlabs/linnkit` repo"路径上的最大 prerequisite 已清。详见 §3 公开 API 边界表（contracts 现在是 SSE/EventEnvelope 系列真源）+ §5.7 v0.1.1 落地说明。**踩坑教训（连撞 2 个，全被 CI 兜底）**：①首次 `git tag linnkit-v0.1.1` 时漏改了 `package.json#version` 还是 0.1.0，CI 在 `Verify tag matches package.json version` step fail（exit 1，1m14s）。②修完 version 重 push 后又踩：D-5 commit `45b953b6` 手抖漏 `git add` 了内化到 `linnkit/contracts` 的 3 个真源新文件 `constants.ts` / `execution.ts` / `sse.ts`（本地工作区有，git 没 track），CI checkout 后 `src/contracts/index.ts` 的 `export * from './sse'` 等 3 行 esbuild 报 `Could not resolve`，CI 在 `Build (tsup → dist)` step fail（2m36s）。两次 fail 都没污染 npm（publish 是 build 之后的步骤）。**修复方式**：补 add 漏的源 + 删除远端旧 tag → 重新打同名 tag → push。**结论**：workflow yml 的两道防线（verify-tag-version + build-before-publish）2026-04-24 同一天连续兑现两次价值，验证它们都不是装饰。后续 release checklist 加两条机械检查放在 §5.7：(a) bump version commit 必须 `git diff HEAD~1 -- package.json` 看到 version 行；(b) `git status --short` 必须为空才能打 tag。
...
- [x] **5.7** 打 tag `linnkit-v0.1.0` → CI 真发 —— 2026-04-23 落地：包已上 `https://github.com/orgs/linnlabs/packages`；`npm view @linnlabs/linnkit --registry=https://npm.pkg.github.com/` 输出 `@linnlabs/linnkit@0.1.0 | UNLICENSED | deps: none | versions: 1 | latest: 0.1.0 | published by BCAutumn`，shasum + sha512 integrity 齐全
  ...
  - **5.7b · v0.1.1 patch release（2026-04-24 落地）**：commit `45b953b6 release: linnkit v0.1.1 (D-5 schemas detach + manifest URL 修正)` 60 files changed / 181+ / 1189- → tag `linnkit-v0.1.1` push → CI 真发。**首发连撞 2 个坑（都被 CI 兜住）**：①tag 推早了（`package.json#version` 还是 0.1.0），workflow yml `Verify tag matches package.json version` step 按设计 fail（exit 1，1m14s）。②修完 version 重 push 后又撞：D-5 commit `45b953b6` 手抖漏 `git add` 了 `linnkit/contracts` 内化的 3 个真源新文件（`constants.ts` / `execution.ts` / `sse.ts`），git 远端没这 3 个文件，CI checkout 后 `src/contracts/index.ts` 的 `export * from './sse'` 等 3 行 esbuild 报 `Could not resolve`，CI 在 `Build (tsup → dist)` step fail（2m36s）。两次 fail npm 上 0.1.1 都没出现，证明 verify-tag + build-before-publish 两道防线都生效。**修复链**：①bump `package.json#version` → `0.1.1` + 同步 `package.shell.test.ts` 断言；②补 add 3 个漏的真源 + commit；③`git push --delete origin linnkit-v0.1.1 && git tag -d linnkit-v0.1.1 && git tag linnkit-v0.1.1 && git push origin linnkit-v0.1.1` → CI 重跑 publish。**Release-time 机械检查清单（2026-04-24 v4 沉淀，避免再踩）**：(a) bump version commit 后 `git diff HEAD~1 -- packages/linnkit/package.json` 必须看到 version 行变化；(b) `git status --short` 必须**完全为空**才允许打 release tag（任何 untracked / modified 都先解决）；(c) 本地先跑 `npm --prefix packages/linnkit run build` 验通过再 push tag。**0.1.1 实质内容**：①D-5 schemas detach round 2（6 处生产代码 `@app/schemas → ../../contracts`，新增 `contracts/{constants,execution,sse}.ts` 三个真源文件、删 `packages/schemas/src/{execution-events,sse-events,sse/*}.ts`、删 `packages/schemas/package.json` 死 subpath exports `./view-models` / `./runtime-models`，加 `schemasDetach.contract.test.ts` 守门）；②manifest URL 三件套修正（homepage / repository.url / bugs.url 由虚构 `linnya/linnya` 改为真实 `BCAutumn/Tingtalk_official_version`）；③CI workflow upgrade（`actions/checkout@v4 → @v5`、`actions/setup-node@v4 → @v5`，修 Node 20 deprecation warning）。**验证**：dist 体积 runtime-kernel.cjs 407 KB → 384 KB（-23 KB，因 SSE/EventEnvelope 不再 fork inline）、contracts.cjs 增厚（成为真源）、`grep -r '@app/schemas' packages/linnkit/dist` 零命中、linnkit 全量单测 + smoke 全绿、linnya host 侧 `import from 'linnkit/contracts'` 抽样无回归
