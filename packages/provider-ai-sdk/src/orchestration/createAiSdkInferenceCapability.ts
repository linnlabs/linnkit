...
export interface AiSdkInferenceCapabilityDependencies {
  ...
  /** 产品级 Provider 错误语义由 Host 注入，adapter 不依赖 Cloud 或账号体系。 */
    ...
    async *stream({ request, route, credential }) {
      ...
      try {
        ...
        const result = streamText({
          ...
          messages: projectCanonicalMessages(request.messages, route),
          // Linnkit canonical contract 保留有序 system message；不能折叠成单个顶层 instructions。
