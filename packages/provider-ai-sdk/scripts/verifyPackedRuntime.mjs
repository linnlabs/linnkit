...
try {
  ...
  assert.equal(packedManifest.private, undefined, '发布制品不得保留 private 标记');
  assert.equal(packedManifest.license, 'MIT', '发布制品缺少 MIT license 声明');
  assert.equal(packedManifest.engines?.node, '>=22', '发布制品 Node engines 与上游不一致');
  assert.equal(packedManifest.publishConfig?.access, 'public', 'scoped package 必须公开发布');
  assert.equal(
